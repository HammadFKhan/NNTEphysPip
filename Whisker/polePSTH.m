function Spikes = polePSTH(Spikes,Behaviour)
...
%% Analyze for passive trials 
% Just copy and paste the same format for each trial structure type.
if isfield(Behaviour,'Pole')
    fprintf('Analyzing passive trials...\n')
    count = 1;
    trials = {};
    for ii = 1:length(Spikes.Clusters)
        if ~isempty(Spikes.Clusters(ii).spikeTime)
            for i = 1:length(Behaviour.Pole)
                st = Behaviour.Pole(i).LFPTime(1);
                stp = Behaviour.Pole(i).LFPTime(end);
    ...
    %%
    Spikes.PSTH.pole.spks = trials;
    Spikes.PSTH.pole.spkRates = output;
end
