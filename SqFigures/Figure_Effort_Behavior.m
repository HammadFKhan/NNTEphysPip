...
for i=1:length(IntanBehaviour.hitTrace)
    plot(time,smoothdata(IntanBehaviour.hitTrace(i).trace(1:5:end)),'Color',[0 0 0 0.2],'LineWidth',1.5);
    hold on;
end
...
hitTrialPullTime = nan(1,length(IntanBehaviour.hitTrace));
for n = 1:length(IntanBehaviour.hitTrace)
    trueTime = IntanBehaviour.hitTrace(n).LFPIndex;
    firstPull = IntanBehaviour.hitTrace(n).pullCount(1);
...
periCatchTrajectory = nan(length(catchTrials),5);
leverTrace = smoothdata([horzcat(IntanBehaviour.hitTrace.trace),horzcat(IntanBehaviour.effortperturbTrace.trace)],'gaussian',10)+min(horzcat(IntanBehaviour.hitTrace.trace),[],'all');
leverTrace = zscore(diff(leverTrace));
figure,plot(leverTrace)
u = mean(leverTrace(:,1:length(IntanBehaviour.hitTrace)),2);
leverTrace  = leverTrace(:,c);
for n = 1:length(catchTrials)
    try
    for trial = -2:2
        v = leverTrace(:,allTrials(3,catchTrials(n)+trial));
        CC = cov(v,u)/(std(v)*std(u));
        periCatchTrajectory(n,trial+3) = CC(1,2);
    end
    catch
        disp('error computing some CC values...')
        continue;
    end
end
figure;
plot(-2:2, abs(periCatchTrajectory)', 'Color', [0.7 0.7 0.7], 'LineWidth', 1); % All catch trials, gray
hold on
plot(-2:2, nanmean(abs(periCatchTrajectory),1), '-o', 'Color',[0.7 0.3 0.7], 'MarkerFaceColor',[0.7 0.3 0.7], 'LineWidth', 1.5); % Mean, thick black line
...
%%
v_traces = sqrt(diff(leverTrace,1,2).^2 + diff(leverTrace,1,2).^2);
% size: [nTrials, nTimePoints-1]
