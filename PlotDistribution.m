function [h,a] = PlotDistribution(var1,var2,varargin)
...
for i = 1:length(var1),
	...
	set(p,'MarkerFaceColor',[.3 .7 .6],'MarkerEdgeColor','w','MarkerSize',5,'LineWidth',0.01);
	if i > 1,
		m1 = min([m1 pm1]);
		M1 = max([M1 pM1]);
		m2 = min([m2 pm2]);
	...
	bv = bar(x1,dist1,1);
	k = mod(i,size(colors,1)+1);
	set(bv,'FaceColor',[.3 .7 .6],'EdgeColor',[.3 .7 .6]);
	set(top,'xtick',[]);
	xlim([m1 M1]);
	...
	bh = barh(x2,dist2,1);
	k = mod(i,size(colors,1)+1);
	set(bh,'FaceColor',[.3 .7 .6],'EdgeColor',[.3 .7 .6]);
	set(right,'ytick',[]);
	ylim([m2 M2]);
