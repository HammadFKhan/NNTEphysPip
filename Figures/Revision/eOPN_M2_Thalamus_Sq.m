...
%     M1eOPN(fileNum).IntanBehaviour = IntanBehaviour;
    ...
    M2eOPN(fileNum).baselineSqEntropy = getSqEntropy(baselineSpikes);
    M2eOPN(fileNum).eOPNSqEntropy = getSqEntropy(eOPNSpikes);
end

%%
dat = [];
for n = [1:length(M2eOPN)]
    try
        dat = vertcat(dat,[M2eOPN(n).baselineSqEntropy.CueHit.SqI(3)',M2eOPN(n).eOPNSqEntropy.CueHit.SqI(3)']);
    catch
        disp("No Sq detected")
        continue
    end
