...
load('Y:\Hammad\Ephys\LeverTask\Data_for_Figures\eOPN\ThalamicWaveSpeedPooled.mat')
...
[p_wilcoxon_paired, h_wilcoxon_paired, stats_wilcoxon_paired] = signrank(eOPN_speed_data, baseline_speed_data);

% --- Paired Sample t-test ---
% Compares the means of two related (paired) samples. It assumes the differences
% between pairs are normally distributed.
[h_ttest_paired, p_ttest_paired, ci_ttest_paired, stats_ttest_paired] = ttest(eOPN_speed_data, baseline_speed_data);
...
scatter(ones(size(baseline_speed_data)), baseline_speed_data, 100, 'filled', ...
...
scatter(2 * ones(size(eOPN_speed_data)), eOPN_speed_data, 100, 'filled', ...
...
for i = 1:numel(baseline_speed_data)
    plot([1, 2], [baseline_speed_data(i), eOPN_speed_data(i)], 'k-', 'LineWidth', 0.5, 'Color', [0.7 0.7 0.7]); % Light gray line
end
...
all_data = [baseline_speed_data, eOPN_speed_data];
...
fprintf('Baseline Data: [%s]\n', num2str(baseline_speed_data));
fprintf('eOPN Data: [%s]\n', num2str(eOPN_speed_data));
