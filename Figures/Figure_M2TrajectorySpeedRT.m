...
redo = 1;
if redo==1
    files = dir(fullfile('Y:\Hammad\Ephys\LeverTask\Data_for_Figures\M2SpikeData','*.mat'));
    ...
    sessionName = [fpath,'\','M2DynamicsPooledRT.mat'];
    save(sessionName,"M2neuralDynamics","fileNum","files","-v7.3"); %,"betaWaves","thetaWaves","gammaWaves",
    clear
    disp('Loading processed data...')
    load('Y:\Hammad\Ephys\LeverTask\Data_for_Figures\TrajectoryDynamics\M2DynamicsPooledRT.mat')
else
...
for fileNum = 1:length(M2neuralDynamics)
    dim = 1;
    try
        rttot = arrayfun(@(x) x.reactionTime,M2neuralDynamics(fileNum).IntanBehaviour.cueHitTrace);
        ...
        hitTrials = 1:length(M2neuralDynamics(fileNum).IntanBehaviour.cueHitTrace);
        ...
        neuralBehaviour(fileNum).hitTrialsSlowTot = squeeze(M2neuralDynamics(fileNum).neuralDynamics.hit.speed.speed(dim,:,hitTrialsSlow));
        neuralBehaviour(fileNum).hitTrialsFastTot = squeeze(M2neuralDynamics(fileNum).neuralDynamics.hit.speed.speed(dim,:,hitTrialsFast));
        neuralBehaviour(fileNum).hitTrialsTot = squeeze(M2neuralDynamics(fileNum).neuralDynamics.hit.speed.speed(dim,:,:));

        leverTrace = arrayfun(@(x) x.trace, M2neuralDynamics(fileNum).IntanBehaviour.cueHitTrace, 'UniformOutput', false);
        ...
        IntanBehaviour = M2neuralDynamics(fileNum).IntanBehaviour;
        ...
        neuralBehaviour(fileNum).neuralVar = M2neuralDynamics(fileNum).neuralDynamics.hit.s;
        neuralBehaviour(fileNum).neuralVarSlow = M2neuralDynamics(fileNum).neuralDynamics.hit.s(:,hitTrialsSlow);
        neuralBehaviour(fileNum).neuralVarFast = M2neuralDynamics(fileNum).neuralDynamics.hit.s(:,hitTrialsFast);

        neuralBehaviour(fileNum).neuralStab = M2neuralDynamics(fileNum).neuralDynamics.hit.stab;
        neuralBehaviour(fileNum).neuralStabSlow = M2neuralDynamics(fileNum).neuralDynamics.hit.stab(:,hitTrialsSlow);
        neuralBehaviour(fileNum).neuralStabFast = M2neuralDynamics(fileNum).neuralDynamics.hit.stab(:,hitTrialsFast);

        neuralBehaviour(fileNum).neuralSpeed = squeeze(M2neuralDynamics(fileNum).neuralDynamics.hit.speed.speed(dim,:,:));
        neuralBehaviour(fileNum).neuralSpeedslow = squeeze(M2neuralDynamics(fileNum).neuralDynamics.hit.speed.speed(dim,:,hitTrialsSlow));
        neuralBehaviour(fileNum).neuralSpeedfast = squeeze(M2neuralDynamics(fileNum).neuralDynamics.hit.speed.speed(dim,:,hitTrialsFast));
    catch
        continue
    end
end
...
neuralVar = arrayfun(@(x) x.neuralVar, neuralBehaviour,'uniformoutput',0);
stabilityHit = horzcat(neuralVar{1:end-1});
reactionTime = arrayfun(@(x) x.reactionTime, neuralBehaviour,'uniformoutput',0);
reactionTime = horzcat(reactionTime{1:end-1});
...
variabilityMIFA = [];
for fileNum = 1:length(M2neuralDynamics)
    [~,variabilityHit{fileNum},stabilityHit{fileNum},~] = getMeanTraj(M2neuralDynamics(fileNum).neuralDynamics.hit.X,1:size(M2neuralDynamics(fileNum).neuralDynamics.hit.X,3),6);
    [~,variabilityMiss{fileNum},stabilityMiss{fileNum},~] = getMeanTraj(M2neuralDynamics(fileNum).neuralDynamics.miss.X,1:size(M2neuralDynamics(fileNum).neuralDynamics.miss.X,3),6);
    
    [~,variabilityMIHit{fileNum},stabilityMIHit{fileNum},~] = getMeanTraj(M2neuralDynamics(fileNum).neuralDynamics.MIhit.X,1:size(M2neuralDynamics(fileNum).neuralDynamics.MIhit.X,3),6);
    [~,variabilityMIFA{fileNum},stabilityMIFA{fileNum},~] = getMeanTraj(M2neuralDynamics(fileNum).neuralDynamics.MIFA.X,1:size(M2neuralDynamics(fileNum).neuralDynamics.MIFA.X,3),6);
end
