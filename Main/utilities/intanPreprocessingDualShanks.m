...
function ds_filename = intanPreprocessingDualShanks(chanMapFile,intandsFlag,activeElectrodes)
...
for idx = 2:L
    ...
    if idx== L %subtract the last second off the recording
        disp(['Adjusting for ' num2str(intanOffset) ' second offset']);
        Intan.amplifier_data = Intan.amplifier_data(activeElectrodes,1:(size(Intan.amplifier_data,2)-data.Fs*intanOffset));
