function Spikes = leverPSTHSq(Spikes,Behaviour)
    ...
    for ii = 1:length(Spikes.Clusters)
        if ~isempty(Spikes.Clusters(ii).spikeTime)
            for i = 1:length(Behaviour.cueHitTrace)
                ...
                trials{count}(i,:) = temp;
                sqNum = mode(Behehaviour.hitTrace.pullCount);
                for sQN = 1:sqNum
                    pl = IntanBehviour.hitTrace(n).pullCount(sQN);
                    Spikes.PSTH.hit.pl(n,sQN) = pl;
                    neurons{i,sQN} = temp(:,(pullIdx(sQN)-win):(pullIdx(sQN)+win));
                end
            end
        else
            trials{count} = [];
...
elseif isfield(Behaviour,'nHit')
    fprintf('Analyzing reward aligned hit trials...\n')
    count = 1;
    trials = {};
    neurons = {};
    lever = {};
    for ii = 1:length(Spikes.Clusters)
        if ~isempty(Spikes.Clusters(ii).spikeTime)
            for i = 1:length(Behaviour.hitTrace)
                leverTemp = Behaviour.hitTrace(i).trace;
                st = Behaviour.hitTrace(i).LFPtime(1);
                stp = Behaviour.hitTrace(i).LFPtime(end);
                temp = zeros(1,length(0:0.001:round((stp-st),3)));
                spiketm = Spikes.Clusters(ii).spikeTime(Spikes.Clusters(ii).spikeTime>=st & Spikes.Clusters(ii).spikeTime<=stp);
                spiked = discretize(spiketm,st:0.001:stp); % 0.001 bin for 1 ms
                if ~isnan(spiked)
    ...
    %%
    output = make_nice_mean_raster(trials,20,0);
    Spikes.PSTH.hit.SqspkRates = [];
    for n = 1:sqNum
        Spikes.PSTH.hit.SqspkRates(:,:,n) = make_nice_mean_raster(neurons(:,n)',20,0);
    end
    Spikes.PSTH.hit.Sqlever = lever;
    %%
    Spikes.PSTH.hit.spks = trials;
    Spikes.PSTH.hit.spkRates = output;
end
...
elseif isfield(Behaviour,'nMiss')
    fprintf('Analyzing miss trials...\n')
    count = 1;
    trials = {};
    for ii = 1:length(Spikes.Clusters)
        if ~isempty(Spikes.Clusters(ii).spikeTime)
            for i = 1:length(Behaviour.missTrace)
                st = floor(Behaviour.missTrace(i).LFPtime(1));
                stp = floor(Behaviour.missTrace(i).LFPtime(end));
                temp = zeros(1,length(0:0.001:(stp-st)));
                spiketm = Spikes.Clusters(ii).spikeTime(Spikes.Clusters(ii).spikeTime>=st & Spikes.Clusters(ii).spikeTime<=stp);
                spiked = discretize(spiketm,st:0.001:stp); % 0.001 bin for 1 ms
                if ~isnan(spiked)
...
if isfield(Behaviour,'MIHitTrace')
    ...
    neurons = {};
    lever = {};
    Spikes.PSTH.MIHit.pl = nan(length(Behaviour.MIHitTrace),Behaviour.SqNum);
    for ii = 1:length(Spikes.Clusters)
        if ~isempty(Spikes.Clusters(ii).spikeTime)
    ...
    for n = 1:sqNum
        Spikes.PSTH.MIHit.SqspkRates(:,:,n) = make_nice_mean_raster(neurons(:,n)',20,0);
    end
    Spikes.PSTH.MIHit.Sqlever = lever;
...
if isfield(Behaviour,'effortperturbTrace')
    fprintf('Analyzing perturbed Hit trials...\n')
    count = 1;
    trials = {};
    neurons = {};
    lever = {};
    Spikes.PSTH.effortperturb.pl = nan(length(Behaviour.effortperturbTrace),Behaviour.SqNum);
    for ii = 1:length(Spikes.Clusters)
        if ~isempty(Spikes.Clusters(ii).spikeTime)
            for i = 1:length(Behaviour.effortperturbTrace)
                st = floor(Behaviour.effortperturbTrace(i).LFPtime(1));
                stp = floor(Behaviour.effortperturbTrace(i).LFPtime(end));
                temp = zeros(1,length(0:0.001:(stp-st)));
                spiketm = Spikes.Clusters(ii).spikeTime(Spikes.Clusters(ii).spikeTime>=st & Spikes.Clusters(ii).spikeTime<=stp);
                spiked = discretize(spiketm,st:0.001:stp); % 0.001 bin for 1 ms
                if ~isnan(spiked)
                ...
                trials{count}(i,:) = temp;
                sqNum = Behaviour.SqNum;
                pullTrial = Behaviour.effortperturbTrace(i).pullCount;
                ...
                for sQN = 1:length(pullTrial)
                    pl = pullTrial(sQN);
                    Spikes.PSTH.effortperturb.pl(i,sQN) = pl;
                    neurons{ii,sQN}(i,:) = temp((pl-500):(pl+500));
                    lever{sQN}(i,:) = leverTemp((pl-500):(pl+500));
                end
            end
