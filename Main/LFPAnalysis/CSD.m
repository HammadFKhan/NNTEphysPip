...
if plotOn
    ...
    if CSDtype == 1; % plots the voltages for the standard CSD method
        ...
        figure() % plots the voltage data for all electrodes
        %         subplot(1,3,1);
        for j = 1:size(data,2);
            plot(xAxis,3*data(:,j)+yAxisM,'k');
            yAxisMCol(1,j) = yAxisM;
            yAxisM = yAxisM-MmV; % moves baseline of plot down by MmV
            hold on
        end
...
%         figure() % plots the CSD data for the 2nd to 2nd last electrode
        subplot(1,3,2);
...
%         figure() % plots the CSD data for all electrodes
        subplot(1,3,2);
        for j = 1:size(CSD,2);
