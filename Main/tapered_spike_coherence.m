function info = tapered_spike_coherence(ndata,ndata2,interval,window,showplot)
...
datapath = [];  % same directory

%******* if data is just a name, load that file, else it is structure
if (isfield(ndata,'label') == 0)
    disp(sprintf('Trying to load data file %s',ndata));
    load(sprintf('%s%s',datapath,ndata));
    disp(sprintf('Using default sustained interval for analysis'));
    % interval = data.sustained;  %must use default interval then
    if (isempty(interval))
      interval = data.sustained(1:(end-1));  %must use default interval then
    else
       if (interval == 1)
           interval = (data.sustained(1)-400):(data.sustained(1));
       end
       if (interval == 2)
           interval = data.sustained(1:400);
       end
       if (interval == 3)
           interval = data.sustained(401:(end-1));
       end
       if (interval == 4)
           interval = (data.sustained(end)+1):(data.sustained(end)+400);
       end
    end
else
    data = ndata;
...
CNUM = 3;  % max(data.attend);   % 2 or 3 conditions, or more?
%*************************
spikea = cell(1,CNUM);   % single unit attended spikes (in sustatined period
for trial = 1:size(data.trials,2)
   cubo = data.attend(trial);
   if (cubo > CNUM)
       continue;
   end
   spikea{cubo} = [spikea{cubo} ; data.trials{trial}(it_su,:)];   
end


%*********** Now repeat the process and load the other data set ****
if (isfield(ndata2,'label') == 0)
    disp(sprintf('Trying to load data file %s',ndata2));
    load(sprintf('%s%s',datapath,ndata2));
    disp(sprintf('Using default sustained interval for analysis'));
   % interval = data.sustained;  %must use default interval then
...
%*************************
spikeb = cell(1,CNUM);   % single unit attended spikes (in sustatined period
for trial = 1:size(data.trials,2)
   cubo = data.attend(trial);
   if (cubo > CNUM)
       continue;
   end
   spikeb{cubo} = [spikeb{cubo} ; data.trials{trial}(it_su,:)];
end
...
if (showplot == 1)
   ...
   xlabel('Frequency (Hz)');
   title(sprintf('%s with %s',ndata,ndata2));
...
colo = [[1,1,1];[0.5,0,0];[0,0,0.5];[0,0.5,0];[0.5,0.5,0]];
colormap(colo);
