function [IntanBehaviour] = readLeverIntanBiPOLESv2(parameters,lfpTime,analog_adc_data,dig_in_data,Behaviour,plotOption)
...
if cue == 1
    ...
    if parameters.BiPOLES == 1
        ...
        % we can segregate based on arduino. This is because no reward was
        % given on opto cue hit
        IntanBehaviour.nOptoCueMiss = size(optocueIndex,2);
    end
end
if parameters.opto == 1
...
if cue == 1
    ...
    for i=1:IntanBehaviour.nCueHit
        ...
        if parameters.opto == 1 && parameters.BiPOLES == 0
        ...
        if parameters.opto == 1 && parameters.BiPOLES == 1
            if IntanBehaviour.optocueTrace(IntanBehaviour.cueHit(i,1)) == 1   %~isempty(find(optoIndex == IntanBehaviour.cueHit(i,1))) 
                IntanBehaviour.cueHitTrace(i).opto = 1;
            else 
                IntanBehaviour.cueHitTrace(i).opto = 0;
            end 
...
if cue == 1
    ...
    for i=1:IntanBehaviour.nCueMiss
        ...
        if parameters.opto == 1 && parameters.BiPOLES == 1
            if IntanBehaviour.optocueTrace(IntanBehaviour.cueMiss(i,1)) == 1  %~isempty(find(optoIndex == IntanBehaviour.cueMiss(i,1))) 
                IntanBehaviour.cueMissTrace(i).opto = 1;
            else
                IntanBehaviour.cueMissTrace(i).opto = 0;
            end 
...
%% Getting all opto cue , first charaterzing all as optocuemiss and then will segregate later
if cue == 1 && parameters.BiPOLES == 1
    IntanBehaviour.optocueMiss = optocueIndex';
    IntanBehaviour.nOptoCueMiss = size(IntanBehaviour.optocueMiss,1);
    
    st1 = IntanBehaviour.optocueMiss(1,1)-parameters.windowBeforeCue*parameters.Fs;
    spend = IntanBehaviour.optocueMiss(end,1)+parameters.windowAfterCue*parameters.Fs;
    if st1 <= 0
        disp('Short opto Cue Miss trial Detected');
        IntanBehaviour.optocueMiss(1) = [];
        IntanBehaviour.nOptoCueMiss = IntanBehaviour.nOptoCueMiss-1;
    end
    if spend >= size(IntanBehaviour.leverTrace,2)
        disp('Short opto Cue Miss trial Detected');
        IntanBehaviour.optocueMiss(end) = [];
        IntanBehaviour.nOptoCueMiss = IntanBehaviour.nOptoCueMiss-1;
    end
    
    for i=1:IntanBehaviour.nOptoCueMiss
        st = IntanBehaviour.optocueMiss(i,1)-parameters.windowBeforeCue*parameters.Fs;
        sp = IntanBehaviour.optocueMiss(i,1)+parameters.windowAfterCue*parameters.Fs;
        IntanBehaviour.optocueMissTrace(i).opto = 1;
        IntanBehaviour.optocueMissTrace(i).trace = IntanBehaviour.leverTrace(st:sp)';
        IntanBehaviour.optocueMissTrace(i).time = (0:1/parameters.Fs:(size(IntanBehaviour.optocueMissTrace(i).trace,1)-1)*1/parameters.Fs)' - parameters.windowBeforeCue;
        IntanBehaviour.optocueMissTrace(i).LFPIndex = ([st:1:sp])';
        IntanBehaviour.optocueMissTrace(i).LFPtime = IntanBehaviour.time(st:sp)';
        if parameters.cool == 1
            IntanBehaviour.optocueMissTrace(i).temp = IntanBehaviour.tempTrace(st + parameters.windowBeforeCue*parameters.Fs);
        end
    end
end


% Segregating optoCueMiss and optoCueHit 

alloptomisstime = vertcat(Behaviour.optocueMissTrace.i0);
alloptomisstime(:,2) = zeros(numel(alloptomisstime),1);
alloptohittime = vertcat(Behaviour.optocueHitTrace.i0);
alloptohittime(:,2) = ones(numel(alloptohittime),1);

% Combine and sort by first column
allopto = [alloptomisstime; alloptohittime];   % 2‑col matrix
allopto = sortrows(allopto,1);                 % sort by col 1

idxMiss = find(allopto(:,2) == 0);   
idxHits  = find(allopto(:,2) == 1);  

IntanBehaviour.optoCueHitTrace = IntanBehaviour.optocueMissTrace(idxHits);
IntanBehaviour.optoCueMissTrace = IntanBehaviour.optocueMissTrace(idxMiss);
...
%% Getting reaction time for opto Cue trials 
if parameters.BiPOLES == 1
    badTrials = [];
    
    for i=1:IntanBehaviour.nOptoCueHit
        f = IntanBehaviour.optoCueHitTrace(i).trace - IntanBehaviour.MIcutoffHit;
        fAbove = f.*(f >= 0);
        fCross = find(diff(fAbove>0)==1);
        cueIndex = (parameters.windowBeforeCue*parameters.Fs) + 1;
        fCross(fCross<cueIndex) = [];
        if isempty(fCross)
            badTrials = [badTrials,i];
            disp('Bad opto trial in Motion Allignment Detected');
        else
            IntanBehaviour.optoCueHitTrace(i).MIIndex = IntanBehaviour.optoCueHitTrace(i).LFPIndex(fCross(1));
            if cue == 1
                IntanBehaviour.optoCueHitTrace(i).cueIndex = IntanBehaviour.optoCueHitTrace(i).LFPIndex(parameters.windowBeforeCue*parameters.Fs+1);
                IntanBehaviour.optoCueHitTrace(i).reactionTime = (1/parameters.Fs)*(IntanBehaviour.optoCueHitTrace(i).MIIndex-IntanBehaviour.optoCueHitTrace(i).cueIndex);
            end
        end
    end
    
    % Removing bad trials
    IntanBehaviour.optoCueHitTrace(badTrials) = [];
    IntanBehaviour.nOptoCueHit = size(IntanBehaviour.optoCueHitTrace,2);
end
