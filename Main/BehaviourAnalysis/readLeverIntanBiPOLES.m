function [IntanBehaviour] = readLeverIntanBiPOLES(parameters,lfpTime,analog_adc_data,dig_in_data,Behaviour,plotOption)
...
elseif strcmp(parameters.experiment,'cue')
    ...
    if parameters.BiPOLES == 1
        cueTrace = dig_in_data(3,:) + dig_in_data(4,:);
        cueTrace_sound = dig_in_data(3,:);
        cueTrace_opto = dig_in_data(4,:);
    end
...
if parameters.opto == 1
    ...
    if parameters.BiPOLES == 1
      IntanBehaviour.cueTrace_sound = downsample(cueTrace_sound,round(intanFs/parameters.Fs),1);
      IntanBehaviour.cueTrace_opto = downsample(cueTrace_opto,round(intanFs/parameters.Fs),1);
    end
...
if cue == 1
    ...
    for i=1:IntanBehaviour.nCueHit
        ...
        if parameters.opto == 1 && parameters.BiPOLES == 1
            if IntanBehaviour.cueTrace_opto(IntanBehaviour.cueHit(i,1)) == 1   %~isempty(find(optoIndex == IntanBehaviour.cueHit(i,1))) 
                IntanBehaviour.cueHitTrace(i).opto = 1;
            else 
                IntanBehaviour.cueHitTrace(i).opto = 0;
            end 
...
if cue == 1
    ...
    for i=1:IntanBehaviour.nCueMiss
        ...
        if parameters.opto == 1 && parameters.BiPOLES == 1
            if IntanBehaviour.cueTrace_opto(IntanBehaviour.cueMiss(i,1)) == 1  %~isempty(find(optoIndex == IntanBehaviour.cueMiss(i,1))) 
                IntanBehaviour.cueMissTrace(i).opto = 1;
            else
                IntanBehaviour.cueMissTrace(i).opto = 0;
            end 
