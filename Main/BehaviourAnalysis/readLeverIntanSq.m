function [IntanBehaviour] = readLeverIntanSq(parameters,lfpTime,leverTrace,dig_in_data,Behaviour,plotOption)
...
% Return number of sequence pulls for the task
...
%     IntanBehaviour.hit(i) = [rewardIndex(i) lfpTime(rewardIndex(i)) rewardIndex(i) lfpTime(rewardIndex(i))];
    ...
    IntanBehaviour.hitTrace(i).pullCount = IntanBehaviour.pullCount(IntanBehaviour.pullCount>=IntanBehaviour.hitTrace(i).LFPIndex(1) & IntanBehaviour.pullCount<=IntanBehaviour.hitTrace(i).LFPIndex(1)+rewardSt)-IntanBehaviour.hitTrace(i).LFPIndex(1);
...
for i=1:IntanBehaviour.nMiss
    ...
    if ~isempty(IntanBehaviour.missTrace(i).pullCount)
        if parameters.perturbEffort == 1
            ...
            effortIdx = IntanBehaviour.missTrace(i).LFPIndex(plIdx)-1;
...
IntanBehaviour.MIcutoffFA = 0.4*(IntanBehaviour.threshold-IntanBehaviour.meanRestingPositionFA) + IntanBehaviour.meanRestingPositionFA;
...
for i=1:IntanBehaviour.nMiss
    ...
    IntanBehaviour. MIFATrace(i).pullCount = IntanBehaviour.pullCount(IntanBehaviour.pullCount>=IntanBehaviour.MIFATrace(i).LFPIndex(1) & IntanBehaviour.pullCount<IntanBehaviour.MIFATrace(i).LFPIndex(end))-IntanBehaviour.MIFATrace(i).LFPIndex(1);
end
