function [Behaviour] = readLeverBiPOLESv2(enpath,enfile,parameters,lfpTime,plotOption)
...
if cue==1 
    ...
    if parameters.BiPOLES == 1
        Behaviour.nOptoCue = B(end,6);
        Behaviour.nOptoCueHit = Behaviour.nOptoHit;
        Behaviour.nOptoCueMiss = Behaviour.nOptoCue-Behaviour.nOptoCueHit;
    end
end   
Behaviour.B = B(2:end,:);
...
% BiPOLE hit
if parameters.BiPOLES == 1
    optohitIndex = find(diff(B(:,7)) == 1) + 1;
    optohitTime = Behaviour.time(optohitIndex);
    if expFlag == 1
        optohitLFPIndex = zeros(Behaviour.nOptoHit,1);
        optohitLFPTime = zeros(Behaviour.nOptoHit,1);
        for i=1:Behaviour.nOptoHit
            optohitLFPIndex(i) = max(find(lfpTime<=optohitTime(i)));
            optohitLFPTime(i) = lfpTime(optohitLFPIndex(i));
        end
        Behaviour.optohit = [optohitIndex optohitTime optohitLFPIndex optohitLFPTime];
    else
        Behaviour.optohit = [optohitIndex optohitTime];
    end
end
...
% For Bipoles 
if cue == 1 && parameters.BiPOLES == 1
    optocueIndex = find(Behaviour.B(:,end) == 5);
    optocueTime = Behaviour.time(optocueIndex);
    if expFlag == 1
        optocueLFPIndex = zeros(Behaviour.nOptoCue,1);
        optocueLFPTime = zeros(Behaviour.nOptoCue,1);
        for i=1:Behaviour.nOptoCue
            optocueLFPIndex(i) = max(find(lfpTime<=optocueTime(i)));
            optocueLFPTime(i) = lfpTime(optocueLFPIndex(i));
        end
        Behaviour.optocue = [optocueIndex optocueTime optocueLFPIndex optocueLFPTime];
    else
        Behaviour.optocue = [optocueIndex optocueTime];
    end
    
    % Getting cueHits and cueMisses 
    optocueHitIndex = zeros(Behaviour.nOptoHit,1);
    optocueHitTime = zeros(Behaviour.nOptoHit,1);
    optocueHitPullIndex = zeros(Behaviour.nOptoHit,1);
    optocueHitPullTime = zeros(Behaviour.nOptoHit,1);
    if expFlag == 1
        optocueHitLFPIndex = zeros(Behaviour.nOptoHit,1);
        optocueHitLFPTime = zeros(Behaviour.nOptoHit,1);
        optocueHitPullLFPIndex = zeros(Behaviour.nOptoHit,1);
        optocueHitPullLFPTime = zeros(Behaviour.nOptoHit,1);
    end

    a = zeros(Behaviour.nOptoHit,1);
    
    for i=1:Behaviour.nOptoHit
        a(i) = max(find(optocueTime<optohitTime(i))); % No need to check for reaction time. If there is a hit, there is a cue
        optocueHitIndex(i) = optocueIndex(a(i));
        optocueHitTime(i) = optocueTime(a(i));
        optocueHitPullIndex(i) = optohitIndex(i);
        optocueHitPullTime(i) = optohitTime(i);
        if expFlag == 1
            optocueHitLFPIndex(i) = optocueLFPIndex(a(i));
            optocueHitLFPTime(i) = optocueLFPTime(a(i));
            optocueHitPullLFPIndex(i) = optohitLFPIndex(i);
            optocueHitPullLFPTime(i) = optohitLFPTime(i);
        end
    end
    
    if expFlag == 1
        Behaviour.optocueHit = [optocueHitIndex optocueHitTime optocueHitLFPIndex optocueHitLFPTime optocueHitPullIndex optocueHitPullTime optocueHitPullLFPIndex optocueHitPullLFPTime];
        Behaviour.optoreactionTime = optocueHitPullLFPTime - optocueHitLFPTime;
        Behaviour.optomeanReactionTime = mean(Behaviour.optoreactionTime,'all');
    else
        Behaviour.optocueHit = [optocueHitIndex optocueHitTime optocueHitPullIndex optocueHitPullTime];
        Behaviour.optoreactionTime = optocueHitPullTime - optocueHitTime;
        Behaviour.optomeanReactionTime = mean(Behaviour.optoreactionTime,'all');
    end
    
    % For cue Miss trials
    Behaviour.optocueMiss = Behaviour.optocue;
    Behaviour.optocueMiss(a,:) = [];
end 
...
% BiPOLEs
if parameters.BiPOLES == 1
    st_hit1 = max(find(Behaviour.time < Behaviour.optohit(1,2)-parameters.windowBeforePull));
    if isempty(st_hit1)
        disp('First opto hit rejected');
        Behaviour.nOptoHit = Behaviour.nOptoHit-1;
        Behaviour.optohit(1,:) = [];
    %     Behaviour.hit(1,:) = [];
    %     Behaviour.nHit = Behaviour.nHit-1;
    end 
    sp_hitend =  max(find(Behaviour.time < Behaviour.optohit(end,2)+parameters.windowAfterPull));
    if isempty(sp_hitend) 
        disp('Last opto hit rejected')
        Behaviour.nOptoHit = Behaviour.nOptoHit-1;
        Behaviour.optohit(end,:) = [];
    end
    
    for i=1:Behaviour.nOptoHit
        Behaviour.optohitTrace(i).i1 = max(find(Behaviour.time < Behaviour.optohit(i,2)-parameters.windowBeforePull));
        Behaviour.optohitTrace(i).i0 = Behaviour.optohit(i,1);
        Behaviour.optohitTrace(i).i2 = max(find(Behaviour.time < Behaviour.optohit(i,2)+parameters.windowAfterPull));
        Behaviour.optohitTrace(i).rawtrace = Behaviour.leverTrace(Behaviour.optohitTrace(i).i1:Behaviour.optohitTrace(i).i2);
        Behaviour.optohitTrace(i).rawtime = Behaviour.time(Behaviour.optohitTrace(i).i1:Behaviour.optohitTrace(i).i2) - Behaviour.time(Behaviour.optohitTrace(i).i1);
        Behaviour.optohitTrace(i).time1 = Behaviour.time(Behaviour.optohitTrace(i).i1:Behaviour.optohitTrace(i).i2);
        Behaviour.optohitTrace(i).t1 = Behaviour.time(Behaviour.optohitTrace(i).i1);
        Behaviour.optohitTrace(i).t0 = Behaviour.optohit(i,2);
        Behaviour.optohitTrace(i).t2 = Behaviour.time(Behaviour.optohitTrace(i).i2);
        if expFlag == 1
            [Behaviour.optohitTrace(i).trace,Behaviour.optohitTrace(i).time] = resample(Behaviour.optohitTrace(i).rawtrace,Behaviour.optohitTrace(i).rawtime,parameters.Fs,'spline');
            if (size(Behaviour.optohitTrace(i).trace,1)<nlength)
                n1 = size(Behaviour.optohitTrace(i).trace,1);
                n2 = nlength;
                Behaviour.optohitTrace(i).trace = [Behaviour.optohitTrace(i).trace;interp1(1:1:n1,Behaviour.optohitTrace(i).trace,[n1+1:1:n2],'linear','extrap')'];
                Behaviour.optohitTrace(i).time = [Behaviour.optohitTrace(i).time;interp1(1:1:n1,Behaviour.optohitTrace(i).time,[n1+1:1:n2],'linear','extrap')'];
            elseif (size(Behaviour.optohitTrace(i).trace,1)>nlength)
                Behaviour.optohitTrace(i).trace(nlength+1:end) = [];
                Behaviour.optohitTrace(i).time(nlength+1:end) = []; 
            end
            Behaviour.optohitTrace(i).LFPTime = Behaviour.optohitTrace(i).time + (Behaviour.optohit(i,4)-(nlengthBeforePull*parameters.ts));
            Behaviour.optohitTrace(i).LFPIndex = ([Behaviour.optohit(i,3)-nlengthBeforePull:1:nlengthBeforePull+Behaviour.optohit(i,3)])';
        end
    end
end 
...
if Behaviour.time(end) > Behaviour.miss(end,2)+parameters.windowAfterPull
...
if cue == 1 && parameters.BiPOLES == 1
    st_cuehit1 = max(find(Behaviour.time < Behaviour.optocueHit(1,2)-parameters.windowBeforeCue));
    if isempty(st_cuehit1)
        disp('First opto cue hit rejected');
        Behaviour.nOptoCueHit = Behaviour.nOptoCueHit-1;
        Behaviour.optocueHit(1,:) = [];
%         Behaviour.nCueHit = Behaviour.nCueHit-1;
%         Behaviour.cueHit(1,:) = [];
    end 
    sp_cuehitend =  max(find(Behaviour.time < Behaviour.optocueHit(end,2)+parameters.windowAfterCue));
    if isempty(sp_cuehitend)
        disp('Last opto cue hit rejected')
        Behaviour.nOptoCueHit = Behaviour.nOptoCueHit-1;
        Behaviour.optocueHit(end,:) = [];
    end
    for i=1:Behaviour.nOptoCueHit
        Behaviour.optocueHitTrace(i).i1 = max(find(Behaviour.time < Behaviour.optocueHit(i,2)-parameters.windowBeforeCue));
        Behaviour.optocueHitTrace(i).i0 = Behaviour.optocueHit(i,1);
        Behaviour.optocueHitTrace(i).i2 = max(find(Behaviour.time < Behaviour.optocueHit(i,2)+parameters.windowAfterCue));
        if isempty(Behaviour.optocueHitTrace(i).i1)
            continue;
        end
        Behaviour.optocueHitTrace(i).rawtrace = Behaviour.leverTrace(Behaviour.optocueHitTrace(i).i1:Behaviour.optocueHitTrace(i).i2);
        Behaviour.optocueHitTrace(i).rawtime = Behaviour.time(Behaviour.optocueHitTrace(i).i1:Behaviour.optocueHitTrace(i).i2) - Behaviour.time(Behaviour.optocueHitTrace(i).i1);
        Behaviour.optocueHitTrace(i).time1 = Behaviour.time(Behaviour.optocueHitTrace(i).i1:Behaviour.optocueHitTrace(i).i2);
        Behaviour.optocueHitTrace(i).t1 = Behaviour.time(Behaviour.optocueHitTrace(i).i1);
        Behaviour.optocueHitTrace(i).t0 = Behaviour.optocueHit(i,2);
        Behaviour.optocueHitTrace(i).t2 = Behaviour.time(Behaviour.optocueHitTrace(i).i2);
        if expFlag == 1
            [Behaviour.optocueHitTrace(i).trace,Behaviour.optocueHitTrace(i).time] = resample(Behaviour.optocueHitTrace(i).rawtrace,Behaviour.optocueHitTrace(i).rawtime,parameters.Fs,'spline');
            if (size(Behaviour.optocueHitTrace(i).trace,1)<nlengthCue)
                n1 = size(Behaviour.optocueHitTrace(i).trace,1);
                n2 = nlengthCue;
                Behaviour.optocueHitTrace(i).trace = [Behaviour.optocueHitTrace(i).trace;interp1(1:1:n1,Behaviour.optocueHitTrace(i).trace,[n1+1:1:n2],'linear','extrap')'];
                Behaviour.optocueHitTrace(i).time = [Behaviour.optocueHitTrace(i).time;interp1(1:1:n1,Behaviour.optocueHitTrace(i).time,[n1+1:1:n2],'linear','extrap')'];
            elseif (size(Behaviour.optocueHitTrace(i).trace,1)>nlength)
                Behaviour.optocueHitTrace(i).trace(nlength+1:end) = [];
                Behaviour.optocueHitTrace(i).time(nlength+1:end) = []; 
            end
            Behaviour.optocueHitTrace(i).LFPTime = Behaviour.optocueHitTrace(i).time + (Behaviour.optocueHit(i,4)-(nlengthBeforeCue*parameters.ts));
            Behaviour.optocueHitTrace(i).LFPIndex = ([Behaviour.optocueHit(i,3)-nlengthBeforeCue:1:nlengthBeforeCue+Behaviour.optocueHit(i,3)])';
        end
    end
    
    % Getting cueMiss traces
    if Behaviour.nOptoCueMiss > 0
        st_cuemiss1 = max(find(Behaviour.time < Behaviour.optocueMiss(1,2)-parameters.windowBeforeCue));
        if isempty(st_cuemiss1)
            disp('First opto cue miss rejected');
            Behaviour.nOptoCueMiss = Behaviour.nOptoCueMiss-1;
            Behaviour.optocueMiss(1,:) = [];
        end 
        % sp_cuemissend =  max(find(Behaviour.time < Behaviour.optocueMiss(end,2)+parameters.windowAfterCue));
        if Behaviour.time(end) > Behaviour.miss(end,2)+parameters.windowAfterPull
            disp('Last opto cue hit rejected')
            Behaviour.nOptoCueMiss = Behaviour.nOptoCueMiss-1;
            Behaviour.optocueMiss(end,:) = [];
        end
        for i=1:Behaviour.nOptoCueMiss
            Behaviour.optocueMissTrace(i).i1 = max(find(Behaviour.time < Behaviour.optocueMiss(i,2)-parameters.windowBeforeCue));
            Behaviour.optocueMissTrace(i).i0 = Behaviour.optocueMiss(i,1);
            Behaviour.optocueMissTrace(i).i2 = max(find(Behaviour.time < Behaviour.optocueMiss(i,2)+parameters.windowAfterCue));
            Behaviour.optocueMissTrace(i).rawtrace = Behaviour.leverTrace(Behaviour.optocueMissTrace(i).i1:Behaviour.optocueMissTrace(i).i2);
            Behaviour.optocueMissTrace(i).rawtime = Behaviour.time(Behaviour.optocueMissTrace(i).i1:Behaviour.optocueMissTrace(i).i2) - Behaviour.time(Behaviour.optocueMissTrace(i).i1);
            Behaviour.optocueMissTrace(i).time1 = Behaviour.time(Behaviour.optocueMissTrace(i).i1:Behaviour.optocueMissTrace(i).i2);
            Behaviour.optocueMissTrace(i).t1 = Behaviour.time(Behaviour.optocueMissTrace(i).i1);
            Behaviour.optocueMissTrace(i).t0 = Behaviour.optocueMiss(i,2);
            Behaviour.optocueMissTrace(i).t2 = Behaviour.time(Behaviour.optocueMissTrace(i).i2);
            if expFlag == 1
                [Behaviour.optocueMissTrace(i).trace,Behaviour.optocueMissTrace(i).time] = resample(Behaviour.optocueMissTrace(i).rawtrace,Behaviour.optocueMissTrace(i).rawtime,parameters.Fs,'spline');
                if (size(Behaviour.optocueMissTrace(i).trace,1)<nlengthCue)
                    n1 = size(Behaviour.optocueMissTrace(i).trace,1);
                    n2 = nlengthCue;
                    Behaviour.optocueMissTrace(i).trace = [Behaviour.optocueMissTrace(i).trace;interp1(1:1:n1,Behaviour.optocueMissTrace(i).trace,[n1+1:1:n2],'linear','extrap')'];
                    Behaviour.optocueMissTrace(i).time = [Behaviour.optocueMissTrace(i).time;interp1(1:1:n1,Behaviour.optocueMissTrace(i).time,[n1+1:1:n2],'linear','extrap')'];
                elseif (size(Behaviour.optocueMissTrace(i).trace,1)>nlength)
                    Behaviour.optocueMissTrace(i).trace(nlength+1:end) = [];
                    Behaviour.optocueMissTrace(i).time(nlength+1:end) = []; 
                end
                Behaviour.optocueMissTrace(i).LFPTime = Behaviour.optocueMissTrace(i).time + (Behaviour.optocueMiss(i,4)-(nlengthBeforeCue*parameters.ts));
                Behaviour.optocueMissTrace(i).LFPIndex = ([Behaviour.optocueMiss(i,3)-nlengthBeforeCue:1:nlengthBeforeCue+Behaviour.optocueMiss(i,3)])';
            end
        end
    end
end
